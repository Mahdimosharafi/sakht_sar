...
$place_posts = get_posts(['post_type'=>'ss_place','post_status'=>'publish','posts_per_page'=>5]);
$places = [];
if ($place_posts) foreach ($place_posts as $post) {
 $cat = get_the_terms($post->ID,'ss_place_category');
 $rating = get_post_meta($post->ID,'_ss_rating',true);
 $places[] = [$post->post_title, $cat && !is_wp_error($cat) ? $cat[0]->name : 'دیدنی', $rating ?: '4.8', sakhtsar_featured_image($post->ID,$place_fallbacks[count($places)][3] ?? $place_fallbacks[0][3]), get_permalink($post->ID)];
}
if (!$places) foreach($place_fallbacks as $p) $places[] = array_merge($p,[ '#places' ]);
...
$trip_posts = get_posts(['post_type'=>'ss_trip','post_status'=>'publish','posts_per_page'=>3]);
$trips=[];
if ($trip_posts) foreach($trip_posts as $i=>$post){ $trips[]=[ $post->post_title, 'مسیر گردشگری', get_post_meta($post->ID,'_ss_locations',true) ?: '۵ مکان', get_post_meta($post->ID,'_ss_distance',true) ?: '۵۰ کیلومتر', get_post_meta($post->ID,'_ss_duration',true) ?: '۴ ساعت', sakhtsar_featured_image($post->ID,$trip_fallbacks[$i][5] ?? $trip_fallbacks[0][5]), get_permalink($post->ID) ]; }
if(!$trips) foreach($trip_fallbacks as $t) $trips[]=array_merge($t,['#trips']);

$event_posts=get_posts(['post_type'=>'ss_event','post_status'=>'publish','posts_per_page'=>3]);
$events=[];
if($event_posts) foreach($event_posts as $post){$date=get_post_meta($post->ID,'_ss_event_date',true);$events[]=[ $date ? wp_date('j',strtotime($date)) : '24', $post->post_title, get_post_meta($post->ID,'_ss_event_location',true) ?: 'رامسر', sakhtsar_featured_image($post->ID,'https://images.unsplash.com/photo-1490750967868-88aa4486c946?auto=format&fit=crop&w=500&q=80'), get_permalink($post->ID) ];}
if(!$events) $events=[['24','جشنواره بهار نارنج','رامسر · بلوار معلم','https://images.unsplash.com/photo-1490750967868-88aa4486c946?auto=format&fit=crop&w=500&q=80','#'],['10','جشنواره غذاهای محلی','رامسر · مرکز شهر','https://images.unsplash.com/photo-1504674900247-0877df9cc836?auto=format&fit=crop&w=500&q=80','#'],['18','جشنواره تابستان رامسر','رامسر · ساحل','https://images.unsplash.com/photo-1500530855697-b586d89ba3ee?auto=format&fit=crop&w=500&q=80','#']];

$mag_posts=get_posts(['post_type'=>'ss_magazine','post_status'=>'publish','posts_per_page'=>3]);
$mag=[];
if($mag_posts) foreach($mag_posts as $post) $mag[]=[ $post->post_title, wp_trim_words(wp_strip_all_tags($post->post_content),7,'…'), sakhtsar_featured_image($post->ID,'https://images.unsplash.com/photo-1500534623283-312aade485b7?auto=format&fit=crop&w=500&q=80'), get_permalink($post->ID) ];
...
$gallery_posts=get_posts(['post_type'=>'ss_gallery','post_status'=>'publish','posts_per_page'=>6]);
$gallery=[]; if($gallery_posts) foreach($gallery_posts as $post) $gallery[]=sakhtsar_featured_image($post->ID,'');
if(!$gallery) $gallery=array_column($places,3);

get_header(); ?>
<main>
<section class="hero" style="background-image:url('<?php echo esc_url($hero); ?>')">
 <button class="hero-arrow next" type="button" aria-label="قبلی">‹</button><button class="hero-arrow prev" type="button" aria-label="بعدی">›</button>
...
<section class="quick-services"><div class="container service-grid"><?php foreach([['🍲','فرهنگ و غذا','غذاهای محلی و فرهنگ رامسر','#'],['🎒','راهنمای سفر','هر آنچه برای سفر نیاز دارید','#'],['▦','رویدادها','جشنواره‌ها و رویدادهای ویژه','#events'],['🧭','مسیرهای گردشگری','بهترین مسیرها برای سفر','#trips'],['🗺','نقشه رامسر','مشاهده نقشه تعاملی','#'] ] as $s): ?><a class="service-card" href="<?php echo esc_url($s[3]); ?>"><div class="service-icon"><?php echo $s[0]; ?></div><div><div class="service-title"><?php echo $s[1]; ?></div><div class="service-desc"><?php echo $s[2]; ?></div></div></a><?php endforeach; ?></div></section>
<section class="section" id="places"><div class="container"><div class="section-head"><h2 class="section-title">جاهای رامسر</h2><a class="section-link" href="<?php echo esc_url(get_post_type_archive_link('ss_place') ?: '#places'); ?>">مشاهده همه ←</a></div><div class="places-wrap"><button class="carousel-btn right" type="button">‹</button><div class="places-grid"><?php foreach($places as $p): ?><a class="place-card" href="<?php echo esc_url($p[4]); ?>"><div class="place-img" style="background-image:url('<?php echo esc_url($p[3]); ?>')"></div><div class="place-body"><h3 class="place-name"><?php echo esc_html($p[0]); ?></h3><div class="place-meta"><span class="rating">★ <?php echo esc_html($p[2]); ?></span><span><?php echo esc_html($p[1]); ?></span></div></div></a><?php endforeach; ?></div><button class="carousel-btn left" type="button">›</button></div></div></section>
