...
<section class="hero" aria-label="معرفی رامسر" style="background-image:url('<?php echo esc_url($hero); ?>')">
 <button class="hero-arrow hero-arrow-next" type="button" aria-label="تصویر بعدی">‹</button>
 <button class="hero-arrow hero-arrow-prev" type="button" aria-label="تصویر قبلی">›</button>
...
<section class="quick-services" aria-label="دسترسی سریع"><div class="container service-grid">
<?php foreach(array(array('🍲','فرهنگ و غذا','غذاهای محلی و فرهنگ رامسر','#magazine'),array('🎒','راهنمای سفر','هر آنچه برای سفر نیاز دارید','#magazine'),array('▦','رویدادها','جشنواره‌ها و رویدادهای ویژه','#events'),array('🧭','مسیرهای گردشگری','بهترین مسیرها برای سفر','#trips'),array('🗺','نقشه رامسر','مشاهده جاهای دیدنی','#places')) as $s): ?>
<a class="service-card" href="<?php echo esc_url($s[3]); ?>"><span class="service-icon"><?php echo esc_html($s[0]); ?></span><span><strong class="service-title"><?php echo esc_html($s[1]); ?></strong><small class="service-desc"><?php echo esc_html($s[2]); ?></small></span></a>
...
<section class="container video-section"><div class="video-banner"><div class="play" aria-hidden="true">▶</div><div class="video-copy"><strong>رامسر را بهتر بشناسید</strong><span>طبیعت، فرهنگ و جاذبه‌های شهر سبز را در یک نگاه ببینید</span><a href="#magazine">تماشای ویدئو</a></div></div></section>
</main>
<?php get_footer(); ?>
